                ...
                @foreach ($category as $key => $val)
                    @if ($val->category_parent == 0)
                        <div class="row p-3 mb-2 bg-primary text-white category-{{ $val->id }}">
                            <div class="col-sm-10">{{ $val->name }}</div>
                            <div class="col-sm-2">
                                @if ($val->status == 1)
                            ...
                            @if ($val2->category_parent == $val->id)
                                <div style="padding:10px 0px 10px 60px" class="category-{{ $val2->id }}">
                                    <div class="row p-3 mb-2 bg-primary text-white ">
                                        <div class="col-sm-10">{{ $val2->name }}</div>
                                        <div class="col-sm-2">
                                            @if ($val2->status == 1)
                                ...
                                @foreach ($category as $key3 => $val3)
                                @if ($val3->category_parent == $val2->id)
                                    <div style="padding:10px 0px 10px 100px" class="category-{{ $val3->id }}">
                                        <div class="row p-3 mb-2 bg-primary text-white ">
                                            <div class="col-sm-10">{{ $val3->name }}</div>
                                            <div class="col-sm-2">
                                                @if ($val2->status == 1)
                                                    <a style="color: white" id="category_status{{ $val3->id }}"
                                                        onclick="category_status({{ $val3->id}})">Hiển thị</a>
                                                @else
                                                    <a style="color: white" id="category_status{{ $val3->id }}"
                                                        onclick="category_status({{ $val3->id}})">Ẩn</a>
                                                @endif /
                                                <a style="color: white"
                                                    href="{{ url('/dashboard/category/edit/' . $val3->id) }}">Sửa</a> / <a
                                                    onclick="delete_category({{ $val3->id }})">Xóa</a>
                                            </div>
                                        </div>
                                    </div>
                                @endif
