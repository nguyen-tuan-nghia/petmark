    ...
    <form id="product_creat" action="{{ URL::to('/dashboard/category/update/' . $cate->id) }}" method="POST">
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control @error('name') is-invalid @enderror" name="name" id="name"
                value="{{ $cate->name }}" onkeyup="ChangeToSlug();" placeholder="Enter name">
            @error('name')
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control @error('slug') is-invalid @enderror" id="slug" name="slug"
                value="{{ $cate->slug }}{{ rand(0, 30) }}" placeholder="Slug">
            @error('slug')
                <div class="alert alert-danger">{{ $message }}</div>
            @enderror
        </div>
        <div class="form-group">
            ...
            <input type="text" class="form-control @error('keywords') is-invalid @enderror" name="keywords"
                value="{{ $cate->keywords }}" data-role="tagsinput">
            @error('keywords')
                <div class="alert alert-danger">{{ $message }}</div>
            @enderror
        </div>
        <div class="form-group">
            ...
            <select class="form-control" name="category_parent">
                <option value="0" @if ($cate->category_parernt == 0) selected @endif>- Danh mục cha</option>
                @if (count($category))
                    @foreach ($category as $key => $val)
                        @if ($val->category_parent == 0)
                            <option value="{{ $val->id }}" @if ($cate->category_parent == $val->id) selected @endif>
                                <strong>- {{ $val->name }}</strong></option>
                            @foreach ($category as $key2 => $val2)
                                @if ($val2->category_parent == $val->id)
                                    <option @if ($cate->category_parent == $val2->id) selected @endif value="{{ $val2->id }}">
                                        ----- {{ $val2->name }}</option>
                                @endif
                            @endforeach
                        @endif
        ...
        <div class="form-group">
            ...
            <select class="form-control  @error('status') is-invalid @enderror" name="status">
                <option @if ($cate->status == 1) selected @endif value="1">Hiện</option>
