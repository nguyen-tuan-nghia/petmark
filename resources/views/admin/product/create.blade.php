...
@section('content')
    ...
    <form id="product_creat"
    action="{{ URL::to('/dashboard/product/store') }}"
method="POST"
    ...
    @enderror
        ...
        <div class="form-group">
            ...
            <select class="form-control  @error('category_id') is-invalid @enderror" name="category_id[]" multiple >
                @if (count($category))
                    @foreach ($category as $key => $val)
                        @if ($val->category_parent == 0)
                            <option selected value="{{ $val->id }}"><strong>--{{ $val->name }}</strong></option>
                                ...
                                @if ($val2->category_parent == $val->id)
                                    ...
                                    @foreach ($category as $key3 => $val3)
                                        @if ($val3->category_parent == $val2->id)
                                            <option value="{{ $val3->id }}">-------{{ $val3->name }}</option>
                                        @endif
                                    @endforeach
                                @endif
                            @endforeach
        ...
        <br><br>
        <button type="submit" class="btn btn-primary" id="submit">Lưu</button>
    </form>
