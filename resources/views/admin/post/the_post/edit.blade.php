...
@section('content')
    ...
    <form id="product_creat" action="{{ URL::to('/dashboard/post/update/'.$post->id) }}" method="POST"
        enctype="multipart/form-data">
        @csrf
        <div class="form-group">
            <label>Tên bài viết</label>
            <input type="text" class="form-control @error('name') is-invalid @enderror" name="name" id="name"
                value="{{ $post->name}}" onkeyup="ChangeToSlug();" placeholder="Enter name">
            @error('name')
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control @error('slug') is-invalid @enderror" id="slug" name="slug"
                value="{{ $post->slug}}{{ rand(0, 30) }}" placeholder="Slug">
            @error('slug')
                <div class="alert alert-danger">{{ $message }}</div>
            @enderror
        </div>
            ...
            <div class="image-upload">
                <label for="file-input">
                    <div class="upload-icon">
                        @if ($post->image!=null)
                            <img class="icon" src="{{ asset('post/'.$post->image) }}">
                        @else
        ...
        <div class="form-group">
            ...
            <select class="form-control" name="category_post_id">
                ...
                <option @if($post->category_post_id==$val->id)
                    selected
                    @endif  value="{{$val->id}}"><strong>- {{ $val->name }}</strong></option>
                @endforeach
            </select>
        ...
        <div class="form-group">
            <label>Tác giả</label>
                <input type="text" class="form-control @error('auth') is-invalid @enderror" id="auth" name="auth"
                    value="{{ $post->auth_name}}">
                @error('auth')
                    <div class="alert alert-danger">{{ $message }}</div>
                @enderror
        </div>
