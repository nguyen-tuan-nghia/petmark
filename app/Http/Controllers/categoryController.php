...
class categoryController extends Controller
{
    public function locgia(Request $request, $id)
    {
        ...
        $product = product::with('gallery')->where('category_id','LIKE','%'.$category->id.'%')->orWhere('category_id','LIKE','%, '.$category->id.'%')
    ...
    public function danhmuc($slug)
    {
        ...
        $product = product::with('gallery')->where('category_id','LIKE','%'.$category->id.'%')->orWhere('category_id','LIKE','%, '.$category->id.'%')->where('status', '!=', 0)->get();
